...
function plan(hp, X, Y, sizeX, sizeY, score, dietime, sudu, boomimage, imagesrc) {
	...
	this.planmove = function() {
		...
		} else if(scores > 1000 && scores <= 2000) {
			this.imagenode.style.top = this.imagenode.offsetTop + this.plansudu + 1 + "px";
		} else if(scores > 2000 && scores <= 3000) {
			this.imagenode.style.top = this.imagenode.offsetTop + this.plansudu + 2 + "px";
		} else if(scores > 3000 && scores <= 4000) {
			this.imagenode.style.top = this.imagenode.offsetTop + this.plansudu + 3 + "px";
		} else if(scores > 4000 && scores <= 5000) {
...
var bianjie = function() {
	...
	if(bodyobjX < 532 || bodyobjX > 884 || bodyobjY < 0 || bodyobjY > 582) {
		if(document.removeEventListener) {
			mainDiv.removeEventListener("mousemove", yidong, true);
		} else if(document.detachEvent) {
			mainDiv.detachEvent("onmousemove", yidong);
...
function start() {
	...
	if(mark == 70) {
		...
		//中飞机
		if(mark1 % 5 == 0) {
			enemys.push(new enemy(2, 25, 274, 46, 60, 20, 100, random(1, 2), "image/中飞机爆炸.gif", "image/enemy3_fly_1.png"));
		}
		//大飞机
		if(mark1 == 20) {
			enemys.push(new enemy(3, 57, 210, 110, 164, 30, 100, 1, "image/大飞机爆炸.gif", "image/enemy2_fly_1.png"));
			mark1 = 0;
		}
		//小飞机
		else {
			enemys.push(new enemy(1, 19, 286, 34, 24, 10, 100, random(1, 3), "image/小飞机爆炸.gif", "image/enemy1_fly_1.png"));
		}
		mark = 0;
	}
	...
	for(var i = 0; i < enemyslen; i++) {
		...
		if(enemys[i].imagenode.offsetTop > 540) {
	...
	if(mark % 30 == 0) {
		bullets.push(new oddbullet(parseInt(selfplan.imagenode.style.left) + 17, parseInt(selfplan.imagenode.style.top) - 10));
	}
